...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  7 11:05:34 2022
    ...
    if data['status'] == 0:
        ...
        if msg.topic==HUMIDITY:
            ...
            if humidity>data['humidity_threshold']:
                print(f'umbral humedad {humidity} superado, cancelando suscripciÃ3n')
                mqttc.unsubscribe(HUMIDITY) # Esto debe ser lo Ãoltimo 
                data['status'] = 0
        elif TEMP in msg.topic:
            temp = int(msg.payload)
            if temp<=data['temp_threshold']:
                print(f'temperatura {temp} por debajo de umbral, cancelando suscripciÃ3n') 
                data['status']=0
